...
namespace ASD
{
    public class FootControllerIK : MonoBehaviour
    {
        ...
        private const float MAX_DISTANCE_POWER = 10;
        ...
        [SerializeField] private float m_MinimalSmoothDistance = 0.005f;
        ...
        private void Smoothing(Limb limb)
        {
            ...
            // if (limb.canReachTarget)
            // {
            //     if (m_DistancePower > 0)
            //     {
            //         // Calcula la distancia entre la última posición y la posición objetivo
            //         float movementDistance = Vector3.Distance(limb.lastLowBonePosition, limb.targetPosition);

            //         // Si la distancia es mayor que un umbral mínimo, aplica Lerp
            //         if (movementDistance > m_MinimalSmoothDistance)
            //         {
            //             // Interpolación lineal entre la última posición y la posición objetivo
            //             limb.lastLowBonePosition = Vector3.Lerp(limb.lastLowBonePosition, limb.targetPosition, Time.deltaTime * m_DistancePower);
            //         }
            //     }

            //     // Aplica un Lerp a la rotación si es necesario
            //     if (m_SmoothingAngle > 0)
            //     {
            //         limb.targetRotation = Quaternion.Lerp(limb.lastLowBoneRotation, limb.targetRotation, Time.deltaTime * m_SmoothingAngle);
            //     }
            // }
